...
export default function Fixtures() {
	...
	useEffect(() => {
		GETapi(
			`https://apiv2.apifootball.com/?action=get_events&from=${startDate}&to=${endDate}&league_id=149&APIkey=5f990ce48ebc7388ba775b57240a09e34a74f3ac19ce1d51ee35839a1e2ce4e2`
		).then((res) => setFixtures(res));
	}, [startDate, endDate]);

	useEffect(() => {
		GETapi(
			`https://apiv2.apifootball.com/?action=get_events&from=${pastDate}&to=${yesterday}&league_id=149&APIkey=5f990ce48ebc7388ba775b57240a09e34a74f3ac19ce1d51ee35839a1e2ce4e2`
				...
				{results.map((result, index) => (
					<div className="block--match" key={index}>
						...
						<div className="d-flex justify-content-between align-items-center">
							...
							<div>
								<span>
									{result?.match_hometeam_name || "No data"}{" "}
								</span>
								<span>
									{result?.match_awayteam_name || "No data"}{" "}
								</span>
							</div>
							<div>
								<span>
				...
				{fixtures.map((fixture, index) => (
					<div className="block--match" key={index}>
						...
						<div className="d-flex justify-content-between align-items-center">
							...
							<div>
								<span>
									{fixture?.match_hometeam_name || "No data"}{" "}
								</span>
								<span>
									{fixture?.match_awayteam_name || "No data"}{" "}
								</span>
							</div>
							<div>
								<span>
